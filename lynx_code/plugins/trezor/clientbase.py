...
from lynx_code.util import UserCancelled, UserFacingException
from lynx_code.keystore import bip39_normalize_passphrase
...
class TrezorClientBase(Logger):
    ...
    def has_usable_connection_with_device(self):
        ...
        try:
            res = self.client.ping("lynx pinging device")
            assert res == "lynx pinging device"
    ...
    def get_passphrase(self):
        if self.creating_wallet:
            msg = _("Enter a passphrase to generate this wallet.  Each time "
                    ...
                    "access the AUDAX coins in the wallet.").format(self.device)
        else:
