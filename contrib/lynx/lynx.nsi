...
;Variables
  ...
  !define PRODUCT_NAME_NO_SPACE "Lynx"
  !define PREV_PROD_NAME "Lynx"
  !define PREV_PROD_NAME2 "Lynx"
  !define PRODUCT_WEB_SITE "https://github.com/enkrypter/Lynx-wallet"
...
;Interface Settings
  ...
  !define MUI_ICON "lynx_code\gui\icons\lynx.ico"
...
Section "${PRODUCT_NAME}" SectionDE
  ...
  ${If} ${PREVINSTDIR} != ""
    ...
    DeleteRegKey HKCU "Software\Classes\lynx"
  ...
  File /r "dist\lynx\*.*"
  File "lynx_code\gui\icons\lynx.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\Lynx-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\Lynx-${PRODUCT_VERSION}.exe" "" "$INSTDIR\Lynx-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\Lynx-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\Lynx-${PRODUCT_VERSION}.exe" 0


  ;Links lynx: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\lynx" "" "URL:audax Protocol"
  WriteRegStr HKCU "Software\Classes\lynx" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\lynx\shell\open\command" "" "$\"$INSTDIR\Lynx-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
;--------------------------------
;Descriptions
LangString DESC_DE ${LANG_ENGLISH} "Lynx Wallet"
